...
class Get30BookContradictionsList extends Command
{
     ...
     * @var string
     */
    protected $signature = 'get:30bookContradictionsList {rowId} {miss?}';
    ...
    public function handle()
    {
        if ($this->argument('miss') && $this->argument('miss') == 1) {
            ...
            try {
                $lastCrawler = CrawlerM::where('name', 'LIKE', 'Contradictions-30book-%')->where('type', 2)->orderBy('end', 'desc')->first();
                if (isset($lastCrawler->end)) {
                ...
                $this->info(" \n ---------- Check  " . $this->argument('rowId') . "     $startC  -> $endC         ---------=-- ");
                $newCrawler = CrawlerM::firstOrCreate(array('name' => 'Contradictions-30book-' . $this->argument('rowId'), 'start' => $startC, 'end' => $endC, 'status' => 1, 'type' => 2));
            } catch (\Exception $e) {
        ...
        if (isset($newCrawler)) {
            ...
            while ($rowId <= $endC) {
                ...
                $book_data = Book30book::where('id', $rowId)->first();
                if (isset($book_data) and !empty($book_data)) {
                    ...
                    Book30book::where('id', $rowId)->update($update_data);
                }

                CrawlerM::where('name', 'Contradictions-30book-' . $this->argument('rowId'))->where('start', $startC)->update(['last' => $rowId]);
                $rowId++;
            }
        }
    }
