...
class GetMajma500000 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:majma500000 {crawlerId} {miss?}';
    ...
    public function handle()
    {
        if ($this->argument('miss') && $this->argument('miss') == 1) {
            try {
                $lastCrawler = CrawlerM::where('name','Crawler-majma500000-' . $this->argument('crawlerId'))->where('status', 1)->orderBy('id', 'DESC')->first();
                if (isset($lastCrawler) AND !empty($lastCrawler)) {
                    $startC = $lastCrawler->last;
                    $endC   = $lastCrawler->end;
                    $this->info(" \n ---------- Create Crawler  " . $this->argument('crawlerId') . "     $startC  -> $endC         ---------=-- ");
                    $newCrawler = CrawlerM::firstOrCreate(array('name' => 'Crawler-majma500000-' . $this->argument('crawlerId'), 'start' => $startC, 'end' => $endC, 'status' => 1, 'type' => 2));
            ...
            try {
                $lastCrawler = CrawlerM::where('name','Crawler-majma500000-' . $this->argument('crawlerId'))->where('status', 2)->orderBy('id', 'desc')->first();
                if (isset($lastCrawler) AND !empty($lastCrawler)) {
        ...
        if (isset($newCrawler)) {
            ...
            while ($recordNumber <= $endC) {
                ...
                // $bar->advance();*/
                CrawlerM::where('name','Crawler-majma500000-'.$this->argument('crawlerId'))->where('start',$startC)->update(['last'=>$recordNumber]);
                $recordNumber++;
            }
            $newCrawler->status = 2;
            $newCrawler->save();
