...
class CorrectIsbnFromMajma3 extends Command
{
    ...
    public function handle()
    {
        ...
        $total = BookirBook::where('xisbn', 'not like', "%-%")->where('check_circulation', 0)->where('xid', '>', 1500000)->where('xid', '<', 2000000)->count();
        try {
            $startC = 0;
            $endC   = $total;
        ...
        if (isset($newCrawler)) {
            ...
            $books = BookirBook::where('xisbn', 'not like', "%-%")->where('check_circulation', 0)->where('xid', '>', 1500000)->where('xid', '<', 2000000)->orderby('xid', 'ASC')->limit(60)->get();
