...
class CorrectIsbnFromMajma2 extends Command
{
    ...
    public function handle()
    {
        if ($this->argument('miss') && $this->argument('miss') == 1) {
            try {
                $lastCrawler = CrawlerM::where('name', 'Correct-Isbn-From-Majma2-' . $this->argument('crawlerId'))->where('status', 1)->orderBy('id', 'DESC')->first();
            ...
            try {
                $lastCrawler = CrawlerM::where('name', 'Correct-Isbn-From-Majma2-' . $this->argument('crawlerId'))->where('status', 2)->orderBy('id', 'desc')->first();
                if (isset($lastCrawler) and !empty($lastCrawler)) {
        ...
        if (isset($newCrawler)) {
            ...
            BookirBook::where('xisbn', 'not like', "%-%")->where('check_circulation',0)->where('xid', '>', 1000000)->orderby('xid', 'ASC')->chunk(2000, function ($books, $startC) {
                foreach ($books as $book) {
                    ...
                    // $bar->advance();*/
                    CrawlerM::where('name', 'Correct-Isbn-From-Majma2-' . $this->argument('crawlerId'))->where('start', $startC)->update(['last' => $recordNumber]);
                }
            });
