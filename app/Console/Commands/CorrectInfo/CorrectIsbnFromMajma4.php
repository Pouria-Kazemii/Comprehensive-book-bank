...
class CorrectIsbnFromMajma4 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:CorrectIsbnFromMajma4 {crawlerId} ';
    ...
    {
        $function_caller = 'Correct-Isbn-From-Majma4';
        $total = BookirBook::where('xisbn', 'not like', "%-%")->where('check_circulation', 0)->where('xid', '>', 2000000)->count();
        try {
            $startC = 0;
            $endC   = $total;
            $this->info(" \n ---------- Create Crawler  " . $this->argument('crawlerId') . "     $startC  -> $endC         ---------=-- ");
            $newCrawler = CrawlerM::firstOrCreate(array('name' => 'Correct-Isbn-From-Majma4-' . $this->argument('crawlerId'), 'start' => $startC, 'end' => $endC, 'status' => 1, 'type' => 2));
        } catch (\Exception $e) {
        ...
        if (isset($newCrawler)) {
            ...
            // BookirBook::whereNotNull('xpageurl2')->where('xisbn', 'not like', "%-%")->where('check_circulation', 0)->where('xid', '>', 2000000)->orderby('xid', 'ASC')->chunk(2000, function ($books) use ($bar, $function_caller, $newCrawler) {
            $books = BookirBook::where('xisbn', 'not like', "%-%")->where('check_circulation', 0)->where('xid', '>', 2000000)->orderby('xid', 'ASC')->limit(60)->get();
