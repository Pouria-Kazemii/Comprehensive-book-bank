...
class CorrectIsbnFromMajma1 extends Command
{
    ...
    public function handle()
    {
        ...
        $total = BookirBook::whereNotNull('xpageurl2')->where('xisbn', 'not like', "%-%")->where('check_circulation', 0)->where('xid', '>', 500000)->where('xid', '<', 1000000)->count();
        try {
            $startC = 0;
            $endC   = $total;
        ...
        if (isset($newCrawler)) {
            ...
            $books = BookirBook::whereNotNull('xpageurl2')->where('xisbn', 'not like', "%-%")->where('check_circulation', 0)->where('xid', '>', 500000)->where('xid', '<', 1000000)->orderby('xid', 'ASC')->limit(60)->get();
