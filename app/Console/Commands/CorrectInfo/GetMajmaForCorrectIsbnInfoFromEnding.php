...
class GetMajmaForCorrectIsbnInfoFromEnding extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'get:GetMajmaForCorrectIsbnInfoFromEnding {crawlerId}';
    ...
    public function handle()
    {
        ...
        $total = BookirBook::/*where('xisbn', 'not like', "%-%")->*/where('xid','<','2007732')->where('check_circulation', 0)->count();
        try {
        ...
        if (isset($newCrawler)) {
            ...
            bookirbook::/*where('xisbn', 'not like', "%-%")->*/where('xid','<','2007732')->where('check_circulation', 0)->orderBy('xid', 'DESC')->chunk(200, function ($books) use ($bar, $newCrawler,$function_caller) {
