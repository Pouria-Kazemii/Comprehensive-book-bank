...
class GetGissomContradictionsList extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            $newCrawler = CrawlerM::firstOrCreate(array('name' => 'Contradictions-gisoom-' . $this->argument('crawlerId'), 'start' => '1', 'end' => $check_count, 'status' => 1));
        } catch (\Exception $e) {
        ...
        try {
            $newCrawler = CrawlerM::firstOrCreate(array('name' => 'Contradictions-UnallowableBook-gisoom-' . $this->argument('rowId'), 'status' => 1));
        } catch (\Exception $e) {
            $this->info(" \n ---------- Check  " . $this->argument('rowId') . "              ------------ ");
        }
